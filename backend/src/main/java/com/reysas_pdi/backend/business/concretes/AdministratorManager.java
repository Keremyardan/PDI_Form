...
public class AdministratorManager implements IAdministratorService {
    ...
    public Administrator update(Long id, Administrator administrator) {
        ...
        return administratorRepo.save(existingAdministrator);
    }

    @Override
    public Administrator getById(Long id) {
        return administratorRepo.findById(id).orElseThrow(()-> new NotFoundException(Msg.NOT_FOUND));
    }

    @Override
    public void delete(Long id) {
        Administrator administrator = administratorRepo.findById(id).orElseThrow(() -> new NotFoundException(Msg.NOT_FOUND));
    ...
    public List<Administrator> findByAdministratorId(Long administratorId) {
        return administratorRepo.findAllByAdministratorId(administratorId);
    }
}
